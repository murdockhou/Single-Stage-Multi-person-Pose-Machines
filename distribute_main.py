...
if __name__ == '__main__':
    ...
    def spm_loss(gt_root_joint, gt_joint_offset, gt_joint_offset_weight, preds):
        ...
        pred_joint_loss = huber_loss(gt_joint_offset * gt_joint_offset_weight,
    ...
    with strategy.scope():
        def train_step(inputs):
            ...
            with tf.GradientTape() as tape:
                preds = model(inputs)
                loss = spm_loss(center_map, kps_map, kps_map_weight, preds) * (1.0 / center_config['batch_size']*len(gpu_ids))
            grads = tape.gradient(loss, model.trainable_variables)
            optimizer.apply_gradients(zip(grads, model.trainable_variables))

            return loss
        @tf.function
        def train_epoch(dist_datasets):
            totoal_loss = 0.0
            train_batchs = 0.0
            for x in dist_datasets:
                per_replica_loss = strategy.experimental_run_v2(train_step, args=(x, ))
                totoal_loss += strategy.reduce(tf.distribute.ReduceOp.SUM, per_replica_loss, axis=None)
                train_batchs += 1
            return totoal_loss, train_batchs

    for epoch in range(epochs):
        total_loss, train_batchs = train_epoch(dist_dataset)
